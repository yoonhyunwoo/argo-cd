    ...
    argoproj.io/Application:
      health.lua: |
        ...
        if obj.metadata.generation ~= nil and obj.status.observedGeneration ~= nil and obj.metadata.generation ~= obj.status.observedGeneration then
          -- The controller has not yet reconciled the current spec generation,
          -- so the recorded health belongs to a previous generation.
          hs.message = "waiting for the controller to observe the current spec generation"
          return hs
        end
...
The generation check in the snippet prevents reporting `Healthy` from status that was recorded before the latest spec update. Right after the parent Application updates a child Application (e.g. new Helm parameters), the child's status still reflects the previous generation, and without this check the parent's sync wave gate can progress prematurely ([#4669](https://github.com/argoproj/argo-cd/issues/4669)). The check requires a version that records `status.observedGeneration`; on versions without the field the condition is skipped and the check degrades to the previous behavior.
